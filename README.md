...
(() => {
  ...
  function getBalloonPoints(balloon) {
    return balloon.r < 16 ? 3 : 1; // små = 3p, store = 1p
  }
  ...
  canvas.addEventListener('pointerdown', e => {
    ...
    for (let b of balloons) {
      if (!b.popping && Math.hypot(mx - b.x, my - b.y) <= b.r) {
        ...
        // Poeng: små ballonger gir mer
        score += getBalloonPoints(b);

        // Reset skuddklokka til 10 sek
        lastPopTimestamp = performance.now();
